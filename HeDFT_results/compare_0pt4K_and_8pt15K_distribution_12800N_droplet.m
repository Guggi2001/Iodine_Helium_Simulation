...
potential_steepness_atom = potential_steepness_molecule;
binding_energy_I_atom = 318.43*k_B/eV*1000;
...
steepness = potential_steepness_atom;
binding_energy = binding_energy_I_atom;
end
...
%figure
yyaxis left
line = plot(r, p_radius(r), 'linestyle', linestyles{index}, 'linewidth', 1.7, 'color', cmap1(index,:));
line.Marker = 'none';
mean_r = trapz(r, r.*p_radius(r))./trapz(r, p_radius(r));
hold on
%vline(mean_r);

bracket_offset = (1- index)*0.02;

plot([mean_r, mean_r], 0.1 + 3*[-0.001, 0.001]+bracket_offset, 'HandleVisibility', 'off', 'color', cmap1(index,:), 'linestyle', '-', 'Marker', 'none');
plot([mean_r, droplet_radius], 0.1+ [0, 0]+bracket_offset, 'HandleVisibility', 'off', 'color', cmap1(index,:), 'linestyle', '-', 'Marker', 'none');
plot([droplet_radius, droplet_radius], 0.1 + 3*[-0.001, 0.001]+bracket_offset, 'HandleVisibility', 'off', 'color', cmap1(index,:), 'linestyle', '-', 'Marker', 'none');
...
ln = plot(r_test, pot/1000*eV/boltzmann_constant, 'linestyle', '-', 'linewidth',  1.4, 'color', cmap2(index,:));
ln.Marker = 'none';
...
exportgraphics(f, '0pt4K_vs_8pt15K_radial_distr.pdf', 'ContentType','vector');

legend('\rho(r)r^2, T=0.4 K', '\rho(r)r^2, T=8.15 K', 'I_2 solv. pot.',  'I solv. pot.')