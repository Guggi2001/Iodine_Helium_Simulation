...
if effusive_dynamics
   ...
   % vf = velocity_factor;
    ...
    plot(res_Iplus_He.r*velocity_factor, res_Iplus_He.radial_distribution / max(res_Iplus_He.radial_distribution ));
    plot(res_Iplus_from_He.r*velocity_factor, res_Iplus_from_He.radial_distribution/max(res_Iplus_from_He.radial_distribution));
    % y = res2.data(:,1);
    % hold on
    % plot(res2.r*velocity_factor, y/max(y))
  leg_fig1 = {'VD TS I+He','VD I+He','VD I+',  'simulation'}
...
%plot(v, g(beta(1), beta(2), v));
    ...
    %plot(res.r*vf_timescan, res.radial_distribution);
        ...
        b_r = res.r*vf_timescan>0 & res.r*vf_timescan<1900;
   ...
   % hold on
   % plot(res_Iplus_He.phi,res_Iplus_He.angular_distribution);
...
figure
scatter(1:length(data_ion.mass_i(:,end)/u), data_ion.mass_i(:,end)/u)
...
legend(leg_fig1)
lines = fig1.Children(2).Children;
...
for i=1:length(lines)
    if ismember(i, [1,2,3])
        ...
        fig1.Children(2).Children(i).YData = y;
    end
...
surf( (res.Y - res.image_center_y)*vf/100, (res.X- res.image_center_x)*vf/100, res.image);
view(90,90)
xlabel('vx / A/ps');
ylabel('vy / A/ps');
pbaspect([1,1,1]);
